...
function(data, X, FILL) {
  ...
  g <- ggplot(DF_all, aes(ymin = ymin, ymax = ymax, xmin = xmin, xmax = xmax, fill = residual)) +
    geom_rect(col = "white") +
    geom_text(aes(x = xposn, label = X), y = 1, size = 3, angle = 90, hjust = 1, show.legend = FALSE) +
    geom_text(aes(x= max(xmax), y = yposn, label = FILL), size = 3, hjust = 1, show.legend = FALSE) +
...
p1 %+% DF_all + 
  geom_text(aes(x = DF_all$xposn, label = X),
            y = 1, angle = 90,
            size = 3, hjust = 1,
            show.legend = FALSE)